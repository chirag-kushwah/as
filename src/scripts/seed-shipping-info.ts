...
const seedDatabase = async () => {
  ...
  try {
    ...
    const shippingInfo = database.collection("shippinginfos")

    // Check if collection is empty
    const count = await shippingInfo.countDocuments()
    if (count === 0) {
      // Insert initial shipping info
      const result = await shippingInfo.insertMany(initialShippingInfo)
      console.log(`${result.insertedCount} shipping info records were inserted`)
    } else {
      console.log(`ShippingInfo collection already has ${count} documents. Skipping seed.`)
    }
  } catch (error) {
